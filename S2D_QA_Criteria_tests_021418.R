##QA script for S2D
##Raga Avanasi 02/09/18 QAed by George Agyeman Badu

##Instruction:
#Save this script in the Working Directory of the S2D run you want to test
#source the script first
#After the S2D run finishes, Call this S2D_QA_tests() function with two arguments: 1) the S2D run file, 2) 1 or 0 to/not to include pass messages in the test report 

# read.control file from S2D script: reads the settings for the current S2D run
...
read.chem.fracs = function(chem.list,puc.list,chem.frac.file, g) {
...
S2D_QA_tests = function(control.file="control_file.txt", print.pass.msg = 0)
  
{
  ...
  if(print.pass.msg==0 || print.pass.msg ==1){
   g             <- read.control.file(control.file)
  run.name      <- g$run.name
  
  annual_file <-paste0("output/S2D/",g$run.name,"/Annual/All_houses_annual.csv") #file with path
  if(file.exists(annual_file))   {annual <- fread(annual_file) 
...
##START:
  ...
  chem.fracs     <- read.chem.fracs(chem.list,puc.list,chem.frac.file, g)
  ...
  for(i in 1: nrow(annual)){
     mass_intake[i] <-  annual$dir.derm.exp[i] + annual$dir.ingest.exp[i] + annual$dir.inhal.mass[i] + annual$ind.derm.exp[i] + annual$ind.ingest.exp[i] + annual$ind.inhal.mass[i] 
    systemic_dose[i] <- annual$dir.derm.abs[i] + annual$dir.ingest.abs[i] + annual$dir.inhal.abs[i] + annual$ind.derm.abs[i] + annual$ind.ingest.abs[i] + annual$ind.inhal.abs[i] 
    #annual$test1[i] <-  mass_intake[i]-systemic_dose[i]
    if ((mass_intake[i]-systemic_dose[i]) < 0) {print(paste("Test 1 fails for house", annual$household[i], "chemical", annual$dtxsid[i], sep = " " ))
    } else  { if(print.pass.msg == 1) print(paste("Test 1 passes for house", annual$household[i], "chemical", annual$dtxsid[i], sep = " " )) }
    #
  }   
  ...
  for(i in 1: nrow(annual)){
    ...
    } else {if(print.pass.msg == 1) print(paste("Test 2 passes for house", annual$household[i], "chemical", annual$dtxsid[i], sep = " " ))}
    #
  } 
    ...
    for (h in 1: length(houses_actually_run)){
      ...
      if(file.exists(prod_chem_file))   prod_chem <- fread(prod_chem_file) else print(paste("prod_chem file does not exist in the output/S2D/",g$run.name,"/Prod_chem folder for house",h))
      ...
      if(house_specific_annual[house_specific_annual$dtxsid == chemicals_not_used[chem]]$total.used >0) {print(paste("Test 3 fails for house", houses_actually_run[h], "chemical", chemicals_not_used[chem] ))
          } else {if(print.pass.msg == 1) print(paste("Test 3 passes for house", houses_actually_run[h], "chemical", chemicals_not_used[chem], sep = " " ))}
        }
      } else {print(paste("Test 3 for house", houses_actually_run[h], "is not valid since each chemical is used by at least one individual in the house", sep = " " ))}
    }
  ...
  for (d in 1: length(houses_actually_run)){ 
    ...
    if(file.exists(diary_file))   diary <- fread(diary_file) 
      diary_prim <-diary[diary$Person_household_index==1,]
      diary_prim_pucs <-unique(diary_prim$Sheds_id)
      if(length(intersect(diary_prim_pucs, puc.list)) != 0) {print(paste("Primary individual in house", houses_actually_run[d], "uses at least 1 PUC, so Test 6 is invalid"))
      } else {
       annual_house <-annual[annual$household == houses_actually_run[d],] 
       for(i in 1: nrow(annual_house)){
         if(annual_house$dir.derm.exp[i] >0 || annual_house$dir.inhal.exp[i] >0 || annual_house$dir.ingest.exp[i] >0) {print(paste("Test 6 fails for house", annual_house$household[i], "chemical", annual_house$dtxsid[i], sep = " " ))
         } else {if(print.pass.msg == 1) print(paste("Test 6 passes for house", annual_house$household[i], "chemical", annual_house$dtxsid[i], sep = " " ))}
       }
      }  
    }
 ...
 ##Tests that can be conducted when only a single PUC is evaluated in the S2D run:
  # create a .csv file with PUCs and a flag for indoor/outdoor
  Compartment_fractions<-read.csv("C:/Users/37700/Desktop/HEM/S2D_QA/input/Compartment_fractions.csv")
  PUC_product_codes <-read.csv("C:/Users/37700/Desktop/HEM/S2D_QA/input/PUC_product_codes.csv")
  ...
  if (length(puc.list)==1){
   ...
   ###Test criteria 4: Chemical with NO dermal application has NO direct dermal exposure. 
    ...
    all.dermal.pucs <- x$source.id
    if(puc.list %in%  all.dermal.pucs) {
      print(paste("The PUC", puc.list,"is a dermal application-based PUC and Test 4 is invalid for this PUC"))
      } else {
        for(i in 1: nrow(annual)){
        if(annual$dir.derm.exp[i] >0) {print(paste("Test 4 fails for house", annual$household[i], "chemical", annual$dtxsid[i], sep = " " ))
        }  else {if(print.pass.msg == 1) print(paste("Test 4 passes for house", annual$household[i], "chemical", annual$dtxsid[i], sep = " " ))}
       }
      }
    ## Test criteria 5: Chemical used outdoors causes no indirect exposure
    if(puc.list %in% Indoor_outdoor$source.id ){
    if(Indoor_outdoor$indoor[Indoor_outdoor$source.id==puc.list] ==1){print(paste("The PUC", puc.list, "is an indoor PUC and Test 5 is invalid for this PUC"))
    } else {
      for(i in 1: nrow(annual)){
        if(annual$ind.derm.exp[i] >0 || annual$ind.inhal.exp[i] >0 || annual$ind.ingest.exp[i] >0) {print(paste("Test 5 fails for house", annual$household[i], "chemical", annual$dtxsid[i], sep = " " ))
         }  else {if(print.pass.msg == 1) print(paste("Test 5 passes for house", annual$household[i], "chemical", annual$dtxsid[i], sep = " " ))}
      
       } 
     } 
    }else {print(paste("The PUC", puc.list," currently does not have a 3 letter 'product code' and Test 5 is invalid for it"))}
  ...
  print("The S2D run is evaluating more than 1 PUC and so cannot conduct Tests 4 and 5")
    }

  } else print(paste("Cannot proceed with the QA Tests: All_houses_annual.csv file does not exist in the output/S2D/",g$run.name,"/Annual folder"))
  sink()
 } else {(print(paste("S2D_QA_general_tests function cannot run if the print.pass.msg argument is", print.pass.msg))) }
